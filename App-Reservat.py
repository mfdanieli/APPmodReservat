...
def concentracao(CARGA, taxa_Cin, taxa_Qin, taxa_Qout):
    ...
    for i in range(n-1):
        ...
        carga_permis[i] = (0.03/1000)*(Qout[i]-Qin[i])  # kg/s
        carga_reserv[i] = (Cout[i])*(Qou[i]t-Qin[i])
        perc_remover[i] = 100*(1-carga_permis[i]/carga_reserv[i])
